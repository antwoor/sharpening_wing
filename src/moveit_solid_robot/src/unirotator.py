...
Original file is located at
    https://colab.research.google.com/drive/16LH27flyj49EosLvoEcnzzwDAH74Z-b_
"""
      ...
      if revers == False:
        ...
        for j in range(2, df.shape[0], 1):
          ...
          for j in range(df.shape[0]-1, 1, -1):
            ...
            # Значение переменной Zpos
            file.write(f'{df.iloc[j,i]},')
            file.write(f'{new_df.iloc[j,i-1] - 180},')
            file.write(f'{1}\n')
...
# Создайте пустой датафрейм для объединенных данных
m_df = pd.DataFrame()

# Переберите ваши датафреймы и добавьте строки в merged_df в нужной последовательности
mj = 0;
for i in range(0, wpk.shape[0]):
  m_df = pd.concat([m_df, pd.DataFrame([wpk.iloc[i]])], ignore_index=True)
  if i == wpk.shape[0]-1:
     for j in range(mj,wpc.shape[0]):
          m_df = pd.concat([m_df, pd.DataFrame([wpc.iloc[j]])], ignore_index=True)
          if j < wpc.shape[0] - 1 and wpc.iloc[j+1,0]>wpc.iloc[j,0]:
             mj = j+1;
             break;
  elif wpk.iloc[i+1,0]<wpk.iloc[i,0]:
     for j in range(mj,wpc.shape[0]):
          m_df = pd.concat([m_df, pd.DataFrame([wpc.iloc[j]])], ignore_index=True)
          if j < wpc.shape[0] - 1 and wpc.iloc[j+1,0]>wpc.iloc[j,0]:
             mj = j+1;
...
# Теперь у вас есть объединенный датафрейм с 6 строками из каждого из исходных датафреймов
m_df = m_df.dropna()
m_df.to_csv('merged_df.txt', index=False, header=False)
